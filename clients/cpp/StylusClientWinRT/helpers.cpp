...
bool helpers::TryParseSigDefinedUuid(guid const& uuid, uint16_t& shortId)
{
...
hstring helpers::GetServiceName(GattDeviceService const& service)
{
	uint16_t shortId;

	guid uuid = service.Uuid();
	// Custom services we know
	if (uuid == guid({ 0x6E400001, 0xB5A3, 0xF393, { 0xE0, 0xA9, 0xE5, 0x0E, 0x24, 0xDC, 0xCA, 0x9E } })) {
		return L"Nordic UART Service";
	}
	else if(uuid == guid({ 0x3e440001, 0xf5bb, 0x357d, { 0x71, 0x9d, 0x17, 0x92, 0x72, 0xe4, 0xd4, 0xd9 } })){
		return L"Olas dummy";
	}
...
hstring helpers::GetCharacteristicName(GattCharacteristic const& characteristic)
{
	uint16_t shortId;
